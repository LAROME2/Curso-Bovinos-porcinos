document.addEventListener("DOMContentLoaded", function () {
  ...
  [r4x, r4y] = location(r4);
  r5 = R5.parentNode.getBoundingClientRect();
  [r5x, r5y] = location(r5);
  F = [F1, F2, F3, F4, F5];
  A = [A1, A2, A3, A4, A5];
    ...
    draggable.addEventListener("touchend", function (e) {
      ...
      switch (ans) {
        ...
        case "A5":
          r5 = R5.parentNode.getBoundingClientRect();
          [r5x, r5y] = location(r5);
          console.log(y, r5.y, r5.height, r5y, Math.abs(y - Math.abs(r5y)));
          if (
            Math.abs(x - Math.abs(r5x)) < dif &&
            Math.abs(y - Math.abs(r5y)) < dif2
          ) {
            draggable.style.position = "static";
            R5.appendChild(draggable);
            draggable.classList.remove("draggable");
            if (!isInViewport(R5)) {
              R5.scrollIntoView(true);
              window.scrollBy(0, -50);
            }
            A5 = true;
            A[4] = true;
      ...
      if (par.substring(1) == ans.substring(1)) {
        ...
        switch (ans) {
          ...
          case "A5":
            ...
            F5.style.display = "none";
            C5.style.display = "block";
            S5.classList.remove("container1");
            break;
        }
      } else {
        switch (par) {
